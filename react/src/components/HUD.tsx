...
export default function HUD({zoomIn, zoomOut, zoomActual, rotateCCW, rotateCW, device, setDevice, deviceList, takeSnapshot, closeSnapshot, showSnapshot, framerate}: Props) {
    const [ deviceMenuSelection, setDeviceMenuSelection ] = useState(0);
    function handleChange(e: any) {
        const index: number = e.target.value;
        setDeviceMenuSelection(index);
        if (deviceList) setDevice(deviceList[index]);
    }
    
    function getIndexOfDevice() {
    ...
    return (
        <div className="HUD">
            <div className="bottomHUD">
                ...
                <button onClick={zoomActual} title="Revert Zoom and Rotation">
                ...
                <button style={{position: 'fixed', right: "10px"}}>{framerate} fps</button>
